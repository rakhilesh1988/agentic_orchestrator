...
def test_structured(p):
    ...
    try:
        ...
        if (
            obj.get("city", "").lower() == "paris"
            and obj.get("country", "").lower() == "france"
        ):
...
def test_caching(p):
    ...
    long_sys = (
        "You are a meticulous geography tutor. Always answer concisely. " * 200
    ).strip()
    body = {
